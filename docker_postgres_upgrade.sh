...
show_help() {
    ...
    echo "                • If the validation passes, the test container is removed; if differences are detected,"
    echo "                  the script will output the differences and exit with an error."
    echo ""
    echo "  upgrade:      Performs a production upgrade after user confirmation."
    echo "                • Closes active connections and stops live services (e.g., Apache and PostgreSQL) only"
    echo "                  after receiving explicit user approval."
    echo "                • Creates a full database dump and a backup of the data directory."
    echo "                • Create a target database volume and updates Docker Compose settings."
